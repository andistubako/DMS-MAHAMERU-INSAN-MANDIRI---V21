...
const regexGet = /apiRouter\.get\("\/outlets", authMiddleware, async \(req: AuthenticatedRequest, res\) => \{[\s\S]*?res\.json\(\{ items: paginated, total: pgOutletsList\.length, page, limit \}\);\n\}\);/g;
